...
async function musicalDown(url) {
  if (!url || !/tiktok\.com/i.test(url)) throw new Error('Invalid TikTok URL');
  const agent = 'Mozilla/5.0 (Linux; Android 15; Mobile) AppleWebKit/537.36 Chrome/130.0.0.0 Mobile Safari/537.36';
  const home = await axios.get('https://musicaldown.com/en', { headers: { 'user-agent': agent }, timeout: 30000 });
  ...
  const result = await axios.post('https://musicaldown.com/download', new URLSearchParams(payload).toString(), {
    headers: {
      'content-type': 'application/x-www-form-urlencoded; charset=UTF-8',
      cookie, origin: 'https://musicaldown.com', referer: 'https://musicaldown.com/en', 'user-agent': agent
    }, timeout: 45000
  });
  ...
  $$('a.download').each((_, el) => {
    ...
    const event = String(a.data('event') || '').replace('_download_click', '').toLowerCase();
    const label = a.text().trim().toLowerCase();
    const type = event.includes('mp3') || label.includes('mp3') || label.includes('music') ? 'audio' : (event.includes('photo') || label.includes('photo') || label.includes('image') ? 'photo' : 'video');
    downloads.push({ type, url: href, label: a.text().trim() });
  });
  const images = downloads.filter(x => x.type === 'photo').map(x => x.url);
  const videos = downloads.filter(x => x.type === 'video').map(x => x.url);
  const audios = downloads.filter(x => x.type === 'audio').map(x => x.url);
  const data = [];
  images.forEach(u => data.push({ type: 'photo', url: u }));
  videos.forEach((u, i) => data.push({ type: i === 0 ? 'nowatermark' : 'watermark', url: u }));
  const music = audios[0] || null;
  if (music) data.push({ type: 'audio', url: music });
  if (!data.length) throw new Error('MusicalDown tidak mengembalikan media');
  const author = $$('.video-author b').text().trim();
  const title = $$('.video-desc').text().trim() || 'TikTok media';
  const bg = $$('.video-header').attr('style') || '';
  const cover = (bg.match(/url\((.*?)\)/) || [])[1] || null;
  return { status: true, source: 'musicaldown', title, author: { nickname: author }, cover, data, music_info: music ? { url: music, title: 'TikTok Music', author: '' } : null };
}

async function tiktokDl(url) {
  // Gunakan provider TikWM terlebih dahulu agar seluruh slide foto dikembalikan.
  
  let primary = null
  try { primary = await tiktokv1(url) } catch (e) {}
  try {
    const md = await musicalDown(url)
    if (md?.status) {
      if (primary?.status) {
        const primaryPhotos = (primary.data || []).filter(x => x?.type === 'photo' && x?.url)
        const mdMusic = md.music_info?.url || (md.data || []).find(x => x?.type === 'audio')?.url
        if (mdMusic) primary.music_info = { ...(primary.music_info || {}), url: mdMusic, title: md.music_info?.title || 'TikTok Music', author: md.music_info?.author || '' }
        if (primaryPhotos.length >= 2 || (primary.data || []).some(x => x?.type !== 'photo')) return primary
      }
      return md
    }
  } catch (e) {}
...
module.exports = { musicalDown, tiktokv1, tiktokv2, tiktokv3, tiktokDl }